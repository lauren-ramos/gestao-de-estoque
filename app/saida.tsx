...
export default function SaidaScreen() {
  ...
  async function salvar() {
    ...
    setSaving(true);
    try {
      const dateValue = dataSaida.trim() || new Date().toISOString().slice(0, 10);

      const fotoUrl = fotoUri
        ? await uploadFile(fotoUri, 'fotos', 'saida').catch(() => fotoUri)
        : undefined;
      ...
      Alert.alert('Salvo!', 'Saída registrada com sucesso.', [
  ...
  function limpar() {
    setInsumo(''); setDetalhe(''); setDataSaida('');
